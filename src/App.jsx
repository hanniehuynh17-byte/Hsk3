...
function getDailyVocab() {
  ...
  const seed = day % Math.max(1, ALL_VOCAB.length);
  const arr = [...ALL_VOCAB];
...
function Flashcard({ cards, seenCount, onSeen, onDone }) {
  ...
  return (
    <div style={{ maxWidth: 520, margin: "0 auto", padding: "1rem" }}>
      ...
      {tab === "write" && (
        <div style={{ background: "var(--color-background-secondary)", borderRadius: 16, padding: 24, textAlign: "center" }}>
          <div style={{ fontSize: 13, color: "var(--color-text-secondary)", marginBottom: 12 }}>Luyện viết chữ Hán</div>
          <div id="hanzi-writer-target" style={{ margin: "0 auto", width: 200, height: 200, border: "0.5px solid var(--color-border-tertiary)", borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center", background: "#fff" }}>
            <div style={{ fontSize: 100, fontFamily: "'Noto Serif SC', serif", color: "#1a1a1a" }}>{card.hanzi}</div>
          </div>
          <div style={{ marginTop: 16, fontSize: 14, color: "var(--color-text-secondary)" }}>Viết theo từng nét: {card.hanzi}</div>
          <div style={{ fontSize: 13, color: "var(--color-text-secondary)", marginTop: 4 }}>{card.pinyin} — {card.meaning}</div>
        </div>
      )}
...
function DailyPractice({ seenCount, onSeen, xp, onXP, onBack }) {
  ...
  const dailyCards = useRef(getDailyVocab());
  ...
  if (mode === "menu") return (
    <div style={{ padding: "1rem" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 24 }}>
        ...
        <div>
          <div style={{ fontSize: 18, fontWeight: 500 }}>Luyện tập hôm nay</div>
          <div style={{ fontSize: 13, color: "var(--color-text-secondary)" }}>20 từ ngẫu nhiên mỗi ngày</div>
        </div>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        {[
          ...
          <button key={m.id} onClick={() => setMode(m.id)} style={{ padding: 20, background: "var(--color-background-secondary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 14, cursor: "pointer", textAlign: "left" }}>
  ...
  if (mode === "flashcard") return <><button onClick={() => setMode("menu")} style={{ margin: "1rem", padding: "6px 12px", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 8, background: "transparent", cursor: "pointer" }}>← Quay lại</button><Flashcard cards={dailyCards.current} seenCount={seenCount} onSeen={onSeen} onDone={done} /></>;
  if (mode === "quiz") return <><button onClick={() => setMode("menu")} style={{ margin: "1rem", padding: "6px 12px", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 8, background: "transparent", cursor: "pointer" }}>← Quay lại</button><Quiz cards={dailyCards.current} onDone={done} /></>;
  if (mode === "match") return <><button onClick={() => setMode("menu")} style={{ margin: "1rem", padding: "6px 12px", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 8, background: "transparent", cursor: "pointer" }}>← Quay lại</button><Match cards={dailyCards.current} onDone={done} /></>;
  if (mode === "typing") return <><button onClick={() => setMode("menu")} style={{ margin: "1rem", padding: "6px 12px", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 8, background: "transparent", cursor: "pointer" }}>← Quay lại</button><TypingPinyin cards={dailyCards.current} onDone={done} /></>;
}
...
function Home({ progress, onNav }) {
  ...
  return (
    <div>
      <div style={{ background: "#1a1a1a", color: "#fff", padding: "2rem 1.5rem 1.5rem", marginBottom: 24 }}>
        <div style={{ fontSize: 13, color: "#888", fontFamily: "'JetBrains Mono', monospace", marginBottom: 8 }}>HSK 3 • Giáo trình chuẩn</div>
        <div style={{ fontSize: 36, fontFamily: "'Noto Serif SC', serif", marginBottom: 4 }}>标准教程</div>
        <div style={{ fontSize: 18, fontFamily: "'Cormorant Garamond', serif", color: "#e8d5c0" }}>Giáo trình chuẩn HSK 3</div>
