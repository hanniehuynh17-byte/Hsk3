...
async function speak(text, lang = "zh-CN") {
  ...
  // Priority: Cove (male) / Juniper (female) alternating — zh-TW or zh-CN
  const cove = voices.find(v => v.name === "Cove" || v.name.includes("Cove"));
  const juniper = voices.find(v => v.name === "Juniper" || v.name.includes("Juniper"));
  const male = voices.find(v => v.lang.startsWith("zh") && (v.name.includes("Male") || v.name.includes("Yunxi") || v.name.includes("male")));
  const female = voices.find(v => v.lang.startsWith("zh") && (v.name.includes("Female") || v.name.includes("Xiaoyi") || v.name.includes("female") || v.name.includes("Ting")));
  const zhAny = voices.filter(v => v.lang.startsWith("zh"));
  speakToggle = (speakToggle + 1) % 2;
  if (speakToggle === 0) {
    u.voice = cove || male || (zhAny[0] || null);
  } else {
    u.voice = juniper || female || (zhAny[1] || zhAny[0] || null);
  }
  u.rate = 0.9;
  u.pitch = speakToggle === 0 ? 0.85 : 1.1;
...
// ─── HANZI WRITER PANEL ───────────────────────────────────────────────────────
// Loads hanzi-writer from CDN, shows stroke-order animation + quiz mode
// Also randomizes the word shown each time tab is opened
function HanziWriterPanel({ hanzi, pinyin, meaning, cards }) {
  const writerRef = useRef(null);
  const containerRef = useRef(null);
  const [writerLoaded, setWriterLoaded] = useState(false);
  const [charIdx, setCharIdx] = useState(0);       // which character of the word
  const [mode, setMode] = useState("animate");      // "animate" | "quiz"
  const [quizResult, setQuizResult] = useState(null); // null | "correct" | "wrong"
  const [randomCard, setRandomCard] = useState(null);

  // Pick a random card from cards array (excluding current) on mount
  useEffect(() => {
  ...
  // Create writer when loaded or char changes
  useEffect(() => {
    if (!writerLoaded || !containerRef.current || !currentChar) return;
    if (writerRef.current) {
      try { writerRef.current = null; } catch {}
    }
    ...
    try {
      ...
      containerRef.current.innerHTML = `<div style="display:flex;align-items:center;justify-content:center;height:220px;font-size:100px;font-family:'Noto Serif SC',serif;color:#1a1a1a">${currentChar}</div>`;
    }
  ...
  return (
    <div style={{ background: "var(--color-background-secondary)", borderRadius: 16, padding: "16px 12px" }}>
      ...
      {/* Char selector nếu từ có nhiều chữ */}
      {chars.length > 1 && (
        <div style={{ display: "flex", gap: 6, marginBottom: 12, justifyContent: "center" }}>
          {chars.map((c, i) => (
            <button key={i} onClick={() => { setCharIdx(i); setMode("animate"); }}
              style={{ width: 40, height: 40, fontSize: 20, fontFamily: "'Noto Serif SC', serif", background: charIdx === i ? "#c0392b" : "var(--color-background-primary)", color: charIdx === i ? "#fff" : "var(--color-text-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 8, cursor: "pointer", touchAction: "manipulation" }}>
              {c}
            </button>
          ))}
        </div>
      ...
      <div style={{ position: "relative", margin: "0 auto", width: 220, height: 220 }}>
        ...
        <div ref={containerRef} style={{ position: "absolute", top: 0, left: 0, width: 220, height: 220 }}>
          {!writerLoaded && (
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%", fontSize: 80, fontFamily: "'Noto Serif SC', serif", color: "#1a1a1a" }}>
              {currentChar}
            </div>
          )}
        </div>
      ...
      {writerLoaded && (
        <div style={{ textAlign: "center", marginTop: 8, fontSize: 12, color: "var(--color-text-secondary)" }}>
          Đang xem: <span style={{ color: "#c0392b", fontWeight: 600 }}>{currentChar}</span> — nét vẽ theo thứ tự đỏ
        </div>
      )}
      {!writerLoaded && (
        <div style={{ textAlign: "center", marginTop: 8, fontSize: 12, color: "var(--color-text-secondary)" }}>
          Đang tải dữ liệu nét bút... (cần kết nối mạng)
        </div>
      )}
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
        <button onClick={replay}
          style={{ flex: 1, padding: "10px 8px", fontSize: 13, background: mode === "animate" ? "#1a1a1a" : "transparent", color: mode === "animate" ? "#fff" : "var(--color-text-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 8, cursor: "pointer", touchAction: "manipulation" }}>
          ▶ Xem animation
        </button>
        <button onClick={startQuiz}
          style={{ flex: 1, padding: "10px 8px", fontSize: 13, background: mode === "quiz" ? "#c0392b" : "transparent", color: mode === "quiz" ? "#fff" : "var(--color-text-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 8, cursor: "pointer", touchAction: "manipulation" }}>
          ✍ Tự viết
        </button>
        <button onClick={() => speak(activeCard.hanzi)}
          style={{ padding: "10px 12px", fontSize: 13, background: "transparent", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 8, cursor: "pointer", touchAction: "manipulation" }}>
          🔊
        </button>
      </div>
      ...
      {mode === "quiz" && !quizResult && (
        <div style={{ marginTop: 10, padding: "10px 12px", background: "#fff8f0", borderRadius: 8, borderLeft: "3px solid #c0392b" }}>
          ...
          <div style={{ fontSize: 12, color: "var(--color-text-secondary)", lineHeight: 1.5 }}>
            Dùng ngón tay vẽ từng nét theo đúng thứ tự.<br/>
            Nét đúng → xanh lá ✓ &nbsp;|&nbsp; Nét sai → rung nhẹ
          </div>
        </div>
      )}
      {quizResult === "correct" && (
        <div style={{ marginTop: 10, padding: "10px 12px", background: "#f0fff4", borderRadius: 8, textAlign: "center" }}>
          ...
          <button onClick={startQuiz} style={{ marginTop: 6, padding: "6px 14px", fontSize: 12, background: "#27ae60", color: "#fff", border: "none", borderRadius: 6, cursor: "pointer" }}>
            Thử lại
          </button>
        </div>
      )}
...
function Flashcard({ cards, seenCount, onSeen, onDone }) {
  ...
  return (
    <div style={{ maxWidth: "100%", padding: "1rem" }}>
      ...
      {tab === "learn" && (
        <div onClick={() => setFlipped(f => !f)} style={{ minHeight: 200, background: flipped ? "#1a1a1a" : "#faf7f2", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 16, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", cursor: "pointer", padding: 32, transition: "background 0.2s", userSelect: "none" }}>
          {!flipped ? (
            <>
              <div style={{ fontSize: 68, fontFamily: "'Noto Serif SC', serif", color: "#1a1a1a", lineHeight: 1.1 }}>{card.hanzi}</div>
    ...
    <div style={{ maxWidth: "100%", padding: "1rem" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {opts.current.map(opt => {
          ...
          return (
            <button key={opt} onClick={() => pick(opt)} style={{ padding: "15px 16px", textAlign: "left", fontSize: 15, background: bg, color, border: "0.5px solid var(--color-border-tertiary)", borderRadius: 10, cursor: "pointer", transition: "background 0.2s", fontFamily: "'Cormorant Garamond', serif" }}>{opt}</button>
          );
        })}
      </div>
  ...
  return (
    <div style={{ maxWidth: "100%", padding: "1rem" }}>
      <div style={{ textAlign: "center", marginBottom: 24 }}>
        ...
        <div style={{ fontSize: 64, fontFamily: "'Noto Serif SC', serif", margin: "16px 0" }}>{card.hanzi}</div>
      ...
      <div style={{ marginBottom: 12 }}>
        <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === "Enter" && check()}
          ...
          style={{ width: "100%", padding: "14px 16px", fontSize: 16, border: `1.5px solid ${result === "correct" ? "#27ae60" : result === "wrong" ? "#c0392b" : "var(--color-border-tertiary)"}`, borderRadius: 10, background: "var(--color-background-primary)", color: "var(--color-text-primary)", outline: "none", fontFamily: "'JetBrains Mono', monospace", boxSizing: "border-box" }} />
      </div>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        <button onClick={() => speak(card.hanzi)} style={{ padding: "10px 16px", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 8, background: "transparent", cursor: "pointer", WebkitTapHighlightColor: "transparent" }}>🔊</button>
            ...
            return (
              <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 10, flexDirection: isA ? "row" : "row-reverse" }}>
                ...
                <div style={{ maxWidth: "72%", background: "var(--color-background-secondary)", borderRadius: isA ? "4px 16px 16px 16px" : "16px 4px 16px 16px", padding: "10px 14px" }}>
                  ...
                  <div style={{ fontSize: 15, fontFamily: "'Noto Serif SC', serif", lineHeight: 1.7 }}>{line.text}</div>
                  <button onClick={() => speak(line.text)} style={{ marginTop: 6, padding: "2px 8px", fontSize: 11, border: "none", background: "transparent", color: "var(--color-text-secondary)", cursor: "pointer", WebkitTapHighlightColor: "transparent" }}>🔊</button>
                </div>
              </div>
            );
          })}
...
function ReadingView({ lesson }) {
  ...
  return (
    <div style={{ padding: "1rem" }}>
      <div style={{ fontSize: 15, lineHeight: 2.0, fontFamily: "'Noto Serif SC', serif", marginBottom: 16, padding: 16, background: "var(--color-background-secondary)", borderRadius: 12 }}>
        {lesson.reading.text}
      </div>
      <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
            ...
            <div key={i} style={{ marginBottom: 16, padding: 14, background: "var(--color-background-secondary)", borderRadius: 10 }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <input value={answers[i] || ""} onChange={e => setAnswers(a => ({ ...a, [i]: e.target.value }))}
                  ...
                  style={{ flex: 1, padding: "10px 12px", fontSize: 16, border: `1.5px solid ${checked[i] === true ? "#27ae60" : checked[i] === false ? "#c0392b" : "var(--color-border-tertiary)"}`, borderRadius: 8, background: "var(--color-background-primary)", color: "var(--color-text-primary)", outline: "none", fontFamily: "'Noto Serif SC', serif" }} />
                <button onClick={() => {
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "1rem 1rem 0", marginBottom: 16 }}>
        <button onClick={onBack} style={{ padding: "8px 14px", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 8, background: "transparent", cursor: "pointer", fontSize: 14, touchAction: "manipulation" }}>← Quay lại</button>
        <div>
      ...
      <div style={{ display: "flex", gap: 0, borderBottom: "0.5px solid var(--color-border-tertiary)", overflowX: "auto", padding: "0 1rem", WebkitOverflowScrolling: "touch", scrollbarWidth: "none" }}>
        {sections.map(s => (
          <button key={s.id} onClick={() => setSection(s.id)} style={{ padding: "12px 14px", fontSize: 13, background: "transparent", color: section === s.id ? "#c0392b" : "var(--color-text-secondary)", border: "none", borderBottom: section === s.id ? "2px solid #c0392b" : "2px solid transparent", cursor: "pointer", whiteSpace: "nowrap" }}>
            {s.label}
          </button>
        ))}
      </div>
      ...
      {section === "proverb" && (
        <div style={{ padding: "1.5rem" }}>
          <div style={{ background: "#1a1a1a", color: "#fff", borderRadius: 16, padding: "1.5rem 1.25rem", textAlign: "center" }}>
...
function DailyPractice({ seenCount, onSeen, xp, onXP, onBack }) {
  ...
  // Mỗi lần chọn mini-game → random bộ từ mới
    ...
    <div style={{ padding: "1rem" }}>
      ...
      <div style={{ marginTop: 20, padding: "12px 14px", background: "var(--color-background-secondary)", borderRadius: 12 }}>
        <div style={{ fontSize: 12, color: "var(--color-text-secondary)", marginBottom: 8, fontFamily: "'JetBrains Mono', monospace" }}>
          BỘ TỪ HIỆN TẠI — {cards.length} từ từ {new Set(cards.map(c => c.lesson)).size} bài
        </div>
  ...
  return (
    <div style={{ padding: "1rem" }}>
      <input value={q} onChange={e => setQ(e.target.value)} placeholder="Tìm theo chữ Hán / pinyin / tiếng Việt..."
        style={{ width: "100%", padding: "12px 16px", fontSize: 16, border: "0.5px solid var(--color-border-tertiary)", borderRadius: 10, background: "var(--color-background-primary)", color: "var(--color-text-primary)", outline: "none", boxSizing: "border-box", WebkitAppearance: "none" }} />
      <div style={{ marginTop: 16 }}>
        {results.map(v => (
          <div key={v.id} onClick={() => speak(v.hanzi)} style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 14px", marginBottom: 8, background: "var(--color-background-secondary)", borderRadius: 10, cursor: "pointer", touchAction: "manipulation" }}>
          ...
          {selected.examples.map((ex, i) => (
            <div key={i} style={{ marginBottom: 12, padding: "10px 14px", background: "var(--color-background-primary)", borderRadius: 8 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <button onClick={() => speak(ex)} style={{ padding: "2px 6px", fontSize: 11, border: "none", background: "transparent", cursor: "pointer", WebkitTapHighlightColor: "transparent" }}>🔊</button>
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <div style={{ padding: "1rem" }}>
      <input value={q} onChange={e => setQ(e.target.value)} placeholder="Tìm ngữ pháp..."
        style={{ width: "100%", padding: "12px 16px", fontSize: 16, border: "0.5px solid var(--color-border-tertiary)", borderRadius: 10, background: "var(--color-background-primary)", color: "var(--color-text-primary)", outline: "none", boxSizing: "border-box", WebkitAppearance: "none", marginBottom: 16 }} />
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {filtered.map(g => (
          <button key={g.id} onClick={() => setSelected(g)} style={{ padding: "15px 14px", textAlign: "left", background: "var(--color-background-secondary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 10, cursor: "pointer", touchAction: "manipulation" }}>
      ...
      <div style={{ background: "#1a1a1a", color: "#fff", padding: "1.5rem 1.25rem 1.25rem", marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "4px 10px", border: "0.5px solid #333", borderRadius: 20 }}>
            ...
            <div style={{ fontSize: 11, color: "#999", fontFamily: "'JetBrains Mono', monospace" }}>by Hạc</div>
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: 16, marginTop: 16 }}>
          <div><div style={{ fontSize: 22, fontWeight: 600, fontFamily: "'JetBrains Mono', monospace" }}>{progress.streak}</div><div style={{ fontSize: 12, color: "#888" }}>Ngày streak 🔥</div></div>
          <div><div style={{ fontSize: 22, fontWeight: 600, fontFamily: "'JetBrains Mono', monospace" }}>{progress.xp}</div><div style={{ fontSize: 12, color: "#888" }}>XP tích lũy ⭐</div></div>
          <div><div style={{ fontSize: 22, fontWeight: 600, fontFamily: "'JetBrains Mono', monospace" }}>{learned}</div><div style={{ fontSize: 12, color: "#888" }}>Từ đã học 📚</div></div>
        </div>
      </div>

      <div style={{ padding: "0 1rem" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 24 }}>
          <button onClick={() => onNav("daily")} style={{ padding: "14px 12px", background: "#c0392b", color: "#fff", border: "none", borderRadius: 14, cursor: "pointer", textAlign: "left", WebkitTapHighlightColor: "transparent" }}>
          ...
          <button onClick={() => onNav("lessons")} style={{ padding: "14px 12px", background: "#1a1a1a", color: "#fff", border: "none", borderRadius: 14, cursor: "pointer", textAlign: "left", WebkitTapHighlightColor: "transparent" }}>
            <div style={{ fontSize: 20 }}>📖</div>
          ...
          <button onClick={() => onNav("tools")} style={{ padding: "14px 12px", background: "#faf7f2", color: "#1a1a1a", border: "0.5px solid #e0d8cc", borderRadius: 14, cursor: "pointer", textAlign: "left", WebkitTapHighlightColor: "transparent" }}>
            <div style={{ fontSize: 20 }}>🔧</div>
...
function LessonsList({ onSelect, onBack }) {
  return (
    <div style={{ padding: "1rem" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {ALL_LESSONS.map(l => (
          <button key={l.id} onClick={() => onSelect(l)} style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 14px", background: "var(--color-background-secondary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 12, cursor: "pointer", textAlign: "left", touchAction: "manipulation" }}>
  ...
  if (tool === "grammar") return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "1rem 1rem 0" }}>
        <button onClick={() => setTool("menu")} style={{ padding: "8px 14px", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 8, background: "transparent", cursor: "pointer", fontSize: 14, touchAction: "manipulation" }}>← Quay lại</button>
  ...
  return (
    <div style={{ padding: "1rem" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        <button onClick={() => setTool("grammar")} style={{ padding: "18px 16px", background: "var(--color-background-secondary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 14, cursor: "pointer", textAlign: "left", WebkitTapHighlightColor: "transparent", touchAction: "manipulation" }}>
        ...
        <button onClick={() => setTool("lookup")} style={{ padding: "18px 16px", background: "var(--color-background-secondary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 14, cursor: "pointer", textAlign: "left", WebkitTapHighlightColor: "transparent", touchAction: "manipulation" }}>
...
export default function App() {
  ...
  return (
    <div style={{ fontFamily: "'Cormorant Garamond', 'Noto Serif SC', serif", minHeight: "100vh", background: "#faf7f2", overflowX: "hidden" }}>
      ...
      `}</style>
      <div style={{ maxWidth: 480, margin: "0 auto", background: "var(--color-background-primary)", minHeight: "100vh", paddingBottom: "calc(68px + env(safe-area-inset-bottom, 0px))" }}>
      ...
      <nav style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: "var(--color-background-primary)", borderTop: "0.5px solid var(--color-border-tertiary)", display: "flex", zIndex: 100, paddingBottom: "env(safe-area-inset-bottom, 0px)" }}>
        {[
          ...
          <button key={t.id} onClick={() => nav(t.id)} style={{ flex: 1, padding: "12px 4px 10px", background: "transparent", border: "none", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 3, WebkitTapHighlightColor: "transparent" }}>
            <span style={{ fontSize: 22, lineHeight: 1 }}>{t.icon}</span>
            <span style={{ fontSize: 10, fontWeight: screen === t.id ? 600 : 400, color: screen === t.id ? "#c0392b" : "var(--color-text-secondary)", fontFamily: "'JetBrains Mono', monospace", letterSpacing: "0.02em" }}>{t.label}</span>
          </button>
        ))}
      </nav>
    </div>
