...
function HanziWriterPanel({ hanzi, pinyin, meaning, cards }) {
  ...
  const [rndCard, setRndCard] = useState(null);

  // Reset về từ đang học khi chuyển sang từ mới
  useEffect(() => { setRndCard(null); setCharIdx(0); }, [hanzi]);

  const activeCard = rndCard || { hanzi, pinyin, meaning };
  const chars = [...activeCard.hanzi].filter(c => c >= '一' && c <= '鿿');
  const currentChar = chars[charIdx] || chars[0] || activeCard.hanzi[0];

  useEffect(() => {
    if (window.HanziWriter) { setLoaded(true); return; }
  ...
  useEffect(() => {
    if (!loaded || !containerRef.current || !currentChar) return;
    containerRef.current.innerHTML = "";
    setQuizResult(null); setMode("animate");
    try {
      const w = window.HanziWriter.create(containerRef.current, currentChar, {
        width: 220, height: 220, padding: 10,
        showOutline: true, strokeColor: "#16A34A", outlineColor: "#e0d0c8",
  ...
  function pickRandom() {
    ...
    setRndCard(cards[Math.floor(Math.random() * cards.length)]);
    setCharIdx(0);
  }
  const g = "#f0e8e0";
  return (
    <div style={{ background: "#E8F4EC", borderRadius: 16, padding: "16px 12px" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
        ...
        <button onClick={pickRandom} style={{ padding: "7px 12px", background: "#1A3A2A", color: "#fff", border: "none", borderRadius: 8, cursor: "pointer", fontSize: 13, touchAction: "manipulation" }}>
          🎲 Random
        </button>
      </div>
      {chars.length > 1 && (
        <div style={{ display: "flex", gap: 6, marginBottom: 12, justifyContent: "center" }}>
          {chars.map((c, i) => (
            <button key={i} onClick={() => { setCharIdx(i); setMode("animate"); }}
              style={{ width: 40, height: 40, fontSize: 20, fontFamily: "'Noto Serif SC', serif",
                background: charIdx === i ? "#c0392b" : "var(--color-background-primary)",
                color: charIdx === i ? "#fff" : "var(--color-text-primary)",
                border: "0.5px solid var(--color-border-tertiary)", borderRadius: 8, cursor: "pointer" }}>
              {c}
            </button>
          ))}
        </div>
      ...
      <div style={{ position: "relative", margin: "0 auto", width: 220, height: 220 }}>
        <svg width="220" height="220" style={{ position: "absolute", top: 0, left: 0 }}>
          ...
          <line x1="110" y1="1" x2="110" y2="219" stroke={g} strokeWidth="1.5" strokeDasharray="4,3"/>
          <line x1="1" y1="110" x2="219" y2="110" stroke={g} strokeWidth="1.5" strokeDasharray="4,3"/>
          <line x1="10" y1="10" x2="210" y2="210" stroke={g} strokeWidth="0.8" strokeDasharray="3,4"/>
          <line x1="210" y1="10" x2="10" y2="210" stroke={g} strokeWidth="0.8" strokeDasharray="3,4"/>
        </svg>
        <div ref={containerRef} style={{ position: "absolute", top: 0, left: 0, width: 220, height: 220 }}>
          {!loaded && <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%", fontSize: 80, fontFamily: "'Noto Serif SC', serif" }}>{currentChar}</div>}
        </div>
        {quizResult === "correct" && (
      ...
      <div style={{ textAlign: "center", marginTop: 8, fontSize: 12, color: "var(--color-text-secondary)" }}>
        {loaded
          ? <><span>Đang xem: </span><span style={{ color: "#16A34A", fontWeight: 600 }}>{currentChar}</span><span> — nét vẽ theo thứ tự đỏ</span></>
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
        ...
        <button onClick={startQuiz} style={{ flex: 1, padding: "10px 8px", fontSize: 13, background: mode === "quiz" ? "#1A3A2A" : "transparent", color: mode === "quiz" ? "#fff" : "var(--color-text-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 8, cursor: "pointer", touchAction: "manipulation" }}>
          ✍ Tự viết
        </button>
      ...
      {mode === "quiz" && !quizResult && (
        <div style={{ marginTop: 10, padding: "10px 12px", background: "#fff8f0", borderRadius: 8, borderLeft: "3px solid #16A34A" }}>
          <div style={{ fontSize: 13, color: "#16A34A", fontWeight: 500 }}>✍ Chế độ tự viết</div>
      ...
      {quizResult === "correct" && (
        <div style={{ marginTop: 10, padding: "10px 12px", background: "#f0fff4", borderRadius: 8, textAlign: "center" }}>
          <div style={{ fontSize: 14, color: "#16A34A", fontWeight: 600 }}>🎉 Viết đúng rồi! Xuất sắc!</div>
          <button onClick={startQuiz} style={{ marginTop: 6, padding: "6px 14px", fontSize: 12, background: "#16A34A", color: "#fff", border: "none", borderRadius: 6, cursor: "pointer" }}>Thử lại</button>
        </div>
      )}
    </div>
  );
...
function Flashcard({ cards, seenCount, onSeen, onDone }) {
  ...
  return (
    <div style={{ maxWidth: 520, margin: "0 auto", padding: "1rem" }}>
      <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
        {["learn", "explain", "write"].map(t => (
          <button key={t} onClick={() => setTab(t)}
            style={{ flex: 1, padding: "8px", fontSize: 13, background: tab === t ? "#1A3A2A" : "transparent", color: tab === t ? "#fff" : "var(--color-text-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 8, cursor: "pointer" }}>
      ...
      {tab === "learn" && (
        <div onClick={() => setFlipped(f => !f)} style={{ minHeight: 260, background: flipped ? "#1A3A2A" : "#F5FAF6", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 16, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", cursor: "pointer", padding: 32, transition: "background 0.2s", userSelect: "none" }}>
          {!flipped ? (
            <>
              ...
              {card.sinoViet && <div style={{ fontSize: 14, color: "#c0392b", marginTop: 8, fontFamily: "'Cormorant Garamond', serif", fontStyle: "italic" }}>{card.sinoViet}</div>}
              <div style={{ fontSize: 13, color: "#aaa", marginTop: 14 }}>nhấn để xem nghĩa</div>
            </>
          ) : (
            <>
              <div style={{ fontSize: 44, fontFamily: "'Noto Serif SC', serif", color: "#fff" }}>{card.hanzi}</div>
              <div style={{ fontSize: 18, color: "#6BA882", marginTop: 8, fontFamily: "'Cormorant Garamond', serif" }}>{card.pinyin}</div>
              {card.sinoViet && <div style={{ fontSize: 14, color: "#c0392b", marginTop: 4, fontFamily: "'Cormorant Garamond', serif", fontStyle: "italic" }}>{card.sinoViet}</div>}
      ...
      {tab === "explain" && (
        <div style={{ background: "#E8F4EC", borderRadius: 16, padding: 24 }}>
          ...
          <div style={{ borderTop: "0.5px solid var(--color-border-tertiary)", paddingTop: 16 }}>
            <div style={{ fontSize: 15, marginBottom: 6 }}><strong>Nghĩa:</strong> {card.meaning}</div>
            {card.sinoViet && <div style={{ fontSize: 14, marginBottom: 10, color: "#c0392b", fontFamily: "'Cormorant Garamond', serif", fontStyle: "italic" }}><strong style={{ color: "var(--color-text-primary)", fontStyle: "normal" }}>Hán Việt:</strong> {card.sinoViet}</div>}
            {card.radical && (
              <div style={{ marginTop: 12, padding: "10px 14px", background: "#fff", borderRadius: 10, border: "0.5px solid #D1E7DA" }}>
            ...
            {card.example && (
              <div style={{ marginTop: 10, padding: "10px 14px", background: "#fff", borderRadius: 10, border: "0.5px solid #D1E7DA" }}>
      ...
      <div style={{ display: "flex", gap: 12, marginTop: 16 }}>
        <button onClick={() => speak(card.hanzi)} style={{ padding: "10px 20px", border: "0.5px solid #D1E7DA", borderRadius: 8, background: "transparent", cursor: "pointer", fontSize: 14 }}>🔊 Nghe</button>
        <button onClick={next} style={{ flex: 1, padding: "10px 20px", background: "#1A3A2A", color: "#fff", border: "none", borderRadius: 8, cursor: "pointer", fontSize: 15, fontWeight: 500 }}>
...
function Quiz({ cards, onDone }) {
  ...
  if (done) return (
    <div style={{ textAlign: "center", padding: 40 }}>
      ...
      <button onClick={onDone} style={{ marginTop: 24, padding: "12px 32px", background: "#1A3A2A", color: "#fff", border: "none", borderRadius: 8, fontSize: 16, cursor: "pointer" }}>Về trang chủ</button>
    </div>
  );
  ...
  return (
    <div style={{ maxWidth: 520, margin: "0 auto", padding: "1rem" }}>
      ...
      <button onClick={() => speak(q?.hanzi)} style={{ marginTop: 16, padding: "8px 16px", border: "0.5px solid #D1E7DA", borderRadius: 8, background: "transparent", cursor: "pointer", fontSize: 14, display: "block" }}>🔊 Nghe phát âm</button>
    </div>
  );
}
    ...
    <div style={{ maxWidth: 480, margin: "0 auto", padding: "1rem" }}>
      ...
      <div style={{ marginBottom: 12 }}>
        <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === "Enter" && check()}
          ...
          style={{ width: "100%", padding: "14px 16px", fontSize: 18, border: `1.5px solid ${result === "correct" ? "#16A34A" : result === "wrong" ? "#C0392B" : "var(--color-border-tertiary)"}`, borderRadius: 10, background: "var(--color-background-primary)", color: "var(--color-text-primary)", outline: "none", fontFamily: "'JetBrains Mono', monospace", boxSizing: "border-box" }} />
      </div>
      {result === "wrong" && <div style={{ color: "#c0392b", fontSize: 14, marginBottom: 8 }}>Sai! Đáp án: {card.pinyin}</div>}
      {result === "correct" && <div style={{ color: "#16A34A", fontSize: 14, marginBottom: 8 }}>Đúng rồi! ✓</div>}
      <div style={{ display: "flex", gap: 8 }}>
        ...
        <button onClick={check} style={{ flex: 1, padding: "12px", background: "#1A3A2A", color: "#fff", border: "none", borderRadius: 8, fontSize: 15, cursor: "pointer" }}>Kiểm tra</button>
      </div>
    </div>
  );
}
...
function ReadingView({ lesson }) {
  ...
  return (
    <div style={{ padding: "1rem" }}>
      <div style={{ fontSize: 15, lineHeight: 2.2, fontFamily: "'Noto Serif SC', serif", marginBottom: 20, padding: 20, background: "#E8F4EC", borderRadius: 12 }}>
        {lesson.reading.text}
      </div>
      <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
        <button onClick={() => speak(lesson.reading.text)} style={{ padding: "10px 16px", border: "0.5px solid #D1E7DA", borderRadius: 8, background: "transparent", cursor: "pointer", fontSize: 14 }}>🔊 Nghe</button>
        <button onClick={() => setShowMeta(v => !v)} style={{ padding: "10px 16px", border: "0.5px solid #D1E7DA", borderRadius: 8, background: "transparent", cursor: "pointer", fontSize: 14 }}>
          {showMeta ? "Ẩn" : "Hiện"} Pinyin & Nghĩa
        </button>
      </div>
      {showMeta && (
        <div style={{ padding: 16, background: "#E8F4EC", borderRadius: 12, borderLeft: "3px solid #16A34A" }}>
  ...
  return (
    <div style={{ padding: "1rem" }}>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ background: "#E8F4EC", borderRadius: 12, padding: 20, marginBottom: 20 }}>
            ...
            <div key={i} style={{ marginBottom: 16, padding: 14, background: "var(--color-background-secondary)", borderRadius: 10 }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                ...
                <button onClick={() => {
                  ...
                  setChecked(c => ({ ...c, [i]: ok }));
                }} style={{ padding: "8px 14px", background: "#1A3A2A", color: "#fff", border: "none", borderRadius: 8, cursor: "pointer", fontSize: 13 }}>
                  Kiểm tra
                </button>
              </div>
              {checked[i] !== undefined && (
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "1rem 1rem 0", marginBottom: 16 }}>
        <button onClick={onBack} style={{ padding: "6px 12px", border: "0.5px solid #D1E7DA", borderRadius: 8, background: "transparent", cursor: "pointer", fontSize: 14 }}>← Quay lại</button>
        <div>
      ...
      <div style={{ display: "flex", gap: 0, borderBottom: "0.5px solid #D1E7DA", overflowX: "auto", padding: "0 1rem" }}>
        {sections.map(s => (
      ...
      {section === "proverb" && (
        <div style={{ padding: "1.5rem" }}>
          <div style={{ background: "#1A3A2A", color: "#fff", borderRadius: 16, padding: "2rem", textAlign: "center" }}>
            <div style={{ fontSize: 48, fontFamily: "'Noto Serif SC', serif", marginBottom: 16 }}>{lesson.proverb.hanzi}</div>
            <div style={{ fontSize: 18, color: "#6BA882", marginBottom: 8, fontFamily: "'Cormorant Garamond', serif" }}>{lesson.proverb.pinyin}</div>
...
function DailyPractice({ seenCount, onSeen, xp, onXP, onBack }) {
  ...
  const MODES = [
    { id: "flashcard", icon: "🃏", label: "Flashcard",    desc: "Lật thẻ học từ" },
    { id: "quiz",      icon: "❓", label: "Trắc nghiệm",  desc: "Chọn nghĩa đúng" },
    { id: "match",     icon: "🔗", label: "Ghép từ",      desc: "Nối Hán với nghĩa" },
    { id: "typing",    icon: "⌨",  label: "Gõ Pinyin",    desc: "Nhập pinyin" },
  ];
  function startMode(m) { setCards(getRandomVocab()); setMode(m); }
  function done() { onXP(20); setMode("menu"); }
  const BackBtn = () => (
    <button onClick={() => setMode("menu")}
      style={{ margin: "1rem", padding: "8px 14px", border: "0.5px solid #D1E7DA", borderRadius: 8, background: "transparent", cursor: "pointer", fontSize: 14, touchAction: "manipulation" }}>
      ← Quay lại
    </button>
  );
  ...
  return (
    <div style={{ padding: "1rem" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 18 }}>
        <button onClick={onBack} style={{ padding: "8px 14px", border: "0.5px solid #D1E7DA", borderRadius: 8, background: "transparent", cursor: "pointer", fontSize: 14, touchAction: "manipulation" }}>← Về trang chủ</button>
        <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        {MODES.map(m => (
          <button key={m.id} onClick={() => startMode(m.id)}
            style={{ padding: "18px 14px", background: "#E8F4EC", border: "0.5px solid #D1E7DA", borderRadius: 14, cursor: "pointer", textAlign: "left", touchAction: "manipulation" }}>
            <div style={{ fontSize: 26 }}>{m.icon}</div>
            <div style={{ fontSize: 15, fontWeight: 500, marginTop: 8 }}>{m.label}</div>
            <div style={{ fontSize: 12, color: "var(--color-text-secondary)", marginTop: 4 }}>{m.desc}</div>
          </button>
        ))}
      </div>
      <div style={{ marginTop: 18, padding: "12px 14px", background: "#E8F4EC", borderRadius: 12 }}>
        ...
        {results.map(v => (
          <div key={v.id} onClick={() => speak(v.hanzi)} style={{ display: "flex", alignItems: "center", gap: 16, padding: "12px 14px", marginBottom: 8, background: "var(--color-background-secondary)", borderRadius: 10, cursor: "pointer" }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              {v.sinoViet && <div style={{ fontSize: 12, color: "#c0392b", fontFamily: "'Cormorant Garamond', serif", fontStyle: "italic" }}>{v.sinoViet}</div>}
...
function Home({ progress, onNav }) {
  ...
  return (
    <div>
      <div style={{ background: "#1A3A2A", color: "#fff", padding: "1.5rem 1.25rem 1.25rem", marginBottom: 20 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          <div style={{ fontSize: 11, color: "#3D7A5A", fontFamily: "'JetBrains Mono', monospace", letterSpacing: "0.08em" }}>HSK 3 • GIÁO TRÌNH CHUẨN</div>
          <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "4px 10px", border: "0.5px solid #2E5A3E", borderRadius: 20 }}>
            <div style={{ width: 18, height: 18, borderRadius: "50%", background: "#16A34A", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 700, color: "#fff", fontFamily: "'JetBrains Mono', monospace" }}>H</div>
            <div style={{ fontSize: 11, color: "#999", fontFamily: "'JetBrains Mono', monospace" }}>by Hannie17</div>
          </div>
        </div>
        <div style={{ fontSize: 30, fontFamily: "'Noto Serif SC', serif", marginBottom: 4, color: "#fff" }}>标准教程</div>
        <div style={{ fontSize: 16, fontFamily: "'Cormorant Garamond', serif", color: "#6BA882" }}>Giáo trình chuẩn HSK 3</div>
      ...
      <div style={{ padding: "0 1rem" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 24 }}>
          <button onClick={() => onNav("daily")} style={{ padding: 16, background: "#1A3A2A", color: "#fff", border: "none", borderRadius: 14, cursor: "pointer", textAlign: "left" }}>
          ...
          <button onClick={() => onNav("tools")} style={{ padding: 16, background: "#F2F7F4", color: "#1a1a1a", border: "0.5px solid #e0d8cc", borderRadius: 14, cursor: "pointer", textAlign: "left" }}>
            <div style={{ fontSize: 20 }}>🔧</div>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ height: 6, background: "var(--color-background-secondary)", borderRadius: 3 }}>
            <div style={{ height: "100%", width: `${(learned / totalVocab) * 100}%`, background: "#1A3A2A", borderRadius: 3, transition: "width 0.5s" }} />
          </div>
        </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {ALL_LESSONS.slice(0, 5).map(l => (
              <button key={l.id} onClick={() => onNav("lesson", l)} style={{ display: "flex", alignItems: "center", gap: 14, padding: "12px 14px", background: "var(--color-background-secondary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 10, cursor: "pointer", textAlign: "left" }}>
                <div style={{ width: 36, height: 36, background: "#1A3A2A", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 14, fontWeight: 600 }}>{l.id}</div>
                <div style={{ flex: 1 }}>
  ...
  return (
    <div style={{ padding: "1rem" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
        <button onClick={onBack} style={{ padding: "6px 12px", border: "0.5px solid #D1E7DA", borderRadius: 8, background: "transparent", cursor: "pointer", fontSize: 14 }}>← Về trang chủ</button>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {ALL_LESSONS.map(l => (
          <button key={l.id} onClick={() => onSelect(l)} style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 16px", background: "var(--color-background-secondary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 12, cursor: "pointer", textAlign: "left" }}>
            <div style={{ width: 40, height: 40, background: "#1A3A2A", borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 15, fontWeight: 600, flexShrink: 0 }}>
              {l.id < 10 ? `0${l.id}` : l.id}
            </div>
  ...
  if (tool === "grammar") return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "1rem 1rem 0" }}>
        <button onClick={() => setTool("menu")} style={{ padding: "6px 12px", border: "0.5px solid #D1E7DA", borderRadius: 8, background: "transparent", cursor: "pointer", fontSize: 14 }}>← Quay lại</button>
...
export default function App() {
  ...
  return (
    <div style={{ fontFamily: "'Cormorant Garamond', 'Noto Serif SC', serif", minHeight: "100vh", background: "#F2F7F4" }}>
      ...
      <nav style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 640, background: "var(--color-background-primary)", borderTop: "0.5px solid #D1E7DA", display: "flex", zIndex: 100 }}>
        {[
          ...
          <button key={t.id} onClick={() => nav(t.id)} style={{ flex: 1, padding: "10px 4px", background: "transparent", border: "none", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
            ...
            <span style={{ fontSize: 10, color: screen === t.id ? "#16A34A" : "var(--color-text-secondary)", fontFamily: "'JetBrains Mono', monospace" }}>{t.label}</span>
          </button>
        ))}
      </nav>
    </div>
